        ...
        >
          ...
          <Animated.View entering={FadeInDown.duration(400)} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col gap-5 mb-6">
              ...
              {imageUri ? (
                <View className="relative w-full h-44 rounded-2xl overflow-hidden border border-slate-200">
                  ...
                  <TouchableOpacity
                    ...
                    activeOpacity={0.8}
                    className="absolute right-3 bottom-3 bg-[#0D9488] p-2.5 rounded-full shadow-md active:scale-95"
                  >
                    <Icon name={{ ios: 'camera.fill', android: 'photo-camera', web: 'photo-camera' }} size={16} tintColor="#FFFFFF" />
                  </TouchableOpacity>
                </View>
              ) : (
                <TouchableOpacity
                  onPress={() => setIsCameraVisible(true)}
                  activeOpacity={0.8}
                  className="w-full h-44 border border-dashed border-[#0D9488]/30 bg-[#0D9488]/5 rounded-2xl flex-col items-center justify-center gap-2 active:scale-[0.98]"
                >
                  <View className="w-12 h-12 rounded-full bg-[#0D9488]/10 items-center justify-center">
                    <Icon name={{ ios: 'camera.fill', android: 'photo-camera', web: 'photo-camera' }} size={24} tintColor="#0D9488" />
                  </View>
                  <Text className="text-[#0D9488] text-sm font-bold">Take Receipt Photo</Text>
                </TouchableOpacity>
              )}
            </View>
            ...
            <TouchableOpacity
              ...
              activeOpacity={0.8}
              className={`w-full py-4 rounded-2xl items-center justify-center bg-[#0D9488] active:scale-95 mt-2 ${
            ...
            >
              {isSubmitting ? (
                ...
                <Text className="text-white text-lg font-bold">Submit Expense</Text>
              )}
            </TouchableOpacity>
          </Animated.View>
          ...
          {isHistoryLoading && (
            <View className="p-6 items-center">
              ...
              <Text className="text-slate-500 text-xs font-bold mt-2">Loading expense history...</Text>
            </View>
          )}
                  ...
                  return (
                    <View key={item.id} className="flex-row items-center justify-between border-b border-slate-50 pb-3">
                      ...
                      <View className="items-end shrink-0">
                        <Text className="text-sm font-extrabold text-[#0D9488]">{displayAmount}</Text>
                        <Text className="text-slate-400 text-[10px] mt-0.5">{formattedTime}</Text>
                      </View>
                    </View>
                  );
                })}
