  ...
  return (
    <ScreenBackground>
      ...
      >
        ...
        >
          {/* Main Expense Form */}
          <Animated.View entering={FadeInDown.duration(400)} className="bg-brand-deep p-6 rounded-3xl border border-brand-petrol/40 shadow-sm flex flex-col gap-5 mb-6">
            {/* 1. Camera Section */}
            <View>
              ...
              {imageUri ? (
                <View className="relative w-full h-44 rounded-2xl overflow-hidden border border-brand-petrol/40">
                  <Image source={{ uri: imageUri }} className="w-full h-full" resizeMode="cover" />
                  <TouchableOpacity
                    onPress={() => setIsCameraVisible(true)}
                    activeOpacity={0.8}
                    className="absolute right-3 bottom-3 bg-brand-petrol p-2.5 rounded-full shadow-md active:scale-95"
                  >
                    <Icon name={{ ios: 'camera.fill', android: 'photo-camera', web: 'photo-camera' }} size={16} tintColor="#C9C2C2" />
                  </TouchableOpacity>
                </View>
              ) : (
                <TouchableOpacity
                  onPress={() => setIsCameraVisible(true)}
                  activeOpacity={0.8}
                  className="w-full h-44 border border-dashed border-brand-petrol/30 bg-brand-petrol/5 rounded-2xl flex-col items-center justify-center gap-2 active:scale-[0.98]"
                >
                  <View className="w-12 h-12 rounded-full bg-brand-petrol/10 items-center justify-center">
                    <Icon name={{ ios: 'camera.fill', android: 'photo-camera', web: 'photo-camera' }} size={24} tintColor="#234C58" />
                  </View>
                  <Text className="text-brand-petrol text-sm font-bold">Take Receipt Photo</Text>
                </TouchableOpacity>
              )}
            </View>
            ...
            {category && (
              <View>
                ...
                <TextInput
                  ...
                  className="w-full bg-brand-darkest text-brand-silver px-4 py-3.5 rounded-2xl border border-brand-petrol/40 text-base font-bold"
                />
              </View>
            )}
            ...
            <View>
              ...
              <TextInput
                ...
                className="w-full bg-brand-darkest text-brand-silver px-4 py-3.5 rounded-2xl border border-brand-petrol/40 text-base font-bold h-24"
              />
            </View>
            ...
            {isSubmitting && uploadProgress > 0 && (
              <View className="flex flex-col gap-1 mt-1">
                <Text className="text-xs text-brand-steel font-bold text-right">
                  Uploading Image: {Math.round(uploadProgress * 100)}%
                </Text>
                <View className="w-full h-2 bg-brand-darkest rounded-full overflow-hidden">
                  <View style={{ width: `${uploadProgress * 100}%` }} className="h-full bg-brand-petrol" />
                </View>
              </View>
            )}
            ...
            <TouchableOpacity
              ...
              activeOpacity={0.8}
              className={`w-full py-4 rounded-2xl items-center justify-center bg-brand-petrol active:scale-95 mt-2 ${
            ...
            >
              {isSubmitting ? (
                <View className="flex-row items-center justify-center gap-2">
                  ...
                  <Text className="text-brand-silver text-lg font-bold">Uploading...</Text>
                </View>
              ) : (
                <Text className="text-brand-silver text-lg font-bold">Submit Expense</Text>
              )}
            </TouchableOpacity>
          </Animated.View>
          ...
          {isHistoryLoading && (
            <View className="p-6 items-center">
              <ActivityIndicator size="small" color="#234C58" />
              <Text className="text-brand-steel text-xs font-bold mt-2">Loading expense history...</Text>
            </View>
          )}
            ...
            <Animated.View entering={FadeInDown.duration(400).delay(100)} className="bg-brand-deep p-5 rounded-3xl border border-brand-petrol/40 shadow-sm flex flex-col gap-4 mb-6">
              ...
              <View className="flex flex-col gap-3">
                {expensesData.expenses.map((item: any) => {
                  ...
                  return (
                    <View key={item.id} className="flex-row items-center justify-between border-b border-brand-petrol/40 pb-3">
                      <View className="flex-row items-center gap-3 flex-1">
                        <Image source={{ uri: item.image_url }} className="w-10 h-10 rounded-lg bg-brand-darkest" />
                        <View className="flex-1 mr-2">
                          <Text className="text-sm font-bold text-brand-silver">{catMeta?.label || item.category}</Text>
                          {item.note ? (
                            <Text className="text-xs text-brand-steel italic mt-0.5" numberOfLines={2}>{item.note}</Text>
                          ) : null}
                        </View>
                      </View>
                      <View className="items-end shrink-0">
                        <Text className="text-sm font-extrabold text-brand-petrol">{displayAmount}</Text>
                        <Text className="text-brand-steel text-[10px] mt-0.5">{formattedTime}</Text>
                      </View>
                    </View>
                  );
                })}
